...
public class LithiumServerTickScheduler<T> extends ServerTickList<T> {
    ...
    public void copyTicks(MutableBoundingBox box, BlockPos pos) {
        ...
        for (NextTickListEntry<T> tick : list) {
            this.addEntry(new NextTickListEntry<>(tick.position.add(pos), tick.getTarget(), tick.field_235017_b_, tick.priority));
        }
    }
    ...
    public void scheduleTick(BlockPos pos, T obj, int delay, TickPriority priority) {
        if (!this.filter.test(obj)) {
            this.addEntry(new NextTickListEntry<>(pos, obj, (long) delay + this.world.getGameTime(), priority));
        }
    }
     ...
     */
    private void addEntry(NextTickListEntry<T> tick) {
